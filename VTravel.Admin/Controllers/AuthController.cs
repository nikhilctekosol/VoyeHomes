    ...
    public class AuthController : Controller
    {
        ...
        public IActionResult Login([FromBody] LoginModel model)
        {
            ...
            var query = string.Format(
                "SELECT id,name_of_user,user_role FROM admin_user WHERE user_name='{0}' AND password_enc='{1}' AND 	is_active='Y'",
            ...
            if (isValid)
            {
                var secretKey = new SymmetricSecurityKey(Encoding.UTF8.GetBytes(General.GetSettingsValue("admin_jwtkey")));
        ...
        public IActionResult RequestPass([FromBody] RequestPassModel model)
        {
            try
            {
                ...
                var query = string.Format(
                "UPDATE admin_user SET password_token='{0}', password_token_expiry='{1}' WHERE user_name='{2}' AND is_active='Y'",
                ...
                string body = General.GetSettingsValue("admin_passwordreset_email_template");
        ...
        public IActionResult ResetPass([FromBody] ResetPassModel model)
        {
            try
            {
                ...
                var query = string.Format(
                "UPDATE admin_user SET password_enc='{0}' WHERE  password_token_expiry > '{1}' AND password_token='{2}' AND is_active='Y'",

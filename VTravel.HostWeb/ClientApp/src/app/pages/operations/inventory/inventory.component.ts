...
export class InventoryComponent implements OnInit {
   ...
   var qty= {
    ...
    for (var i = 0; i < this.inventories.length; i++) {
      ...
      if ((invDateA == invDateB) && (roomId == this.inventories[i].roomId)) {
        ...
        qty.totalQty = this.inventories[i].totalQty;
        qty.bookedQty = this.inventories[i].bookedQty;
  ...
  onInvQtyChange(totalQty, invDate, roomId) {


    this.loadingSave = true;

    this.invData = new InvData();
    this.invData.totalQty = parseInt(totalQty);
